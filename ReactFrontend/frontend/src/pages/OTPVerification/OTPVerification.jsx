...
export default function OTPVerification() {
    ...
    return (<motion.div className="page-container auth-page" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="card glass auth-card">
        <h2>Verify OTP</h2>
        <p className="muted">Enter the 6-digit code sent to your phone/email</p>
        <div className="otp-row">
          {otp.map((v, i) => (<input key={i} id={`otp-${i}`} className="otp-input" maxLength={1} value={v} onChange={e => handleChange(i, e.target.value)}/>))}
        </div>
